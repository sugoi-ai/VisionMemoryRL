...
class ActorCriticFFNetwork(ActorCriticNetwork):
  ...
  def __init__(self,
    ...
    with tf.device(self._device):
      ...
      # target position
      self.t = tf.placeholder("float", [None, 4, 4])
      ...
      with tf.variable_scope(network_scope):
        ...
        self.t_flat = tf.reshape(self.t, [-1, 16])
        ...
        h_fc1 = tf.concat(values=[h_s_flat, h_c_flat, self.t_flat], axis=1)

        # shared fusion layer
        self.W_fc2[key] = self._fc_weight_variable([1040, 512])
        self.b_fc2[key] = self._fc_bias_variable([512], 1040)
  ...
  def run_policy_and_value(self, sess, state, target, checkpoint, scopes):
    k = self._get_key(scopes[:2])
    pi_out, v_out = sess.run( [self.pi[k], self.v[k]], feed_dict = {self.s : [state], self.t: [target], self.c: [checkpoint]} )
    return (pi_out[0], v_out[0])

  def run_policy(self, sess, state, target, checkpoint, scopes):
    k = self._get_key(scopes[:2])
    pi_out = sess.run( self.pi[k], feed_dict = {self.s : [state], self.t: [target], self.c: [checkpoint]} )
    return pi_out[0]

  def run_value(self, sess, state, target, checkpoint, scopes):
    k = self._get_key(scopes[:2])
    v_out = sess.run( self.v[k], feed_dict = {self.s : [state], self.t: [target], self.c: [checkpoint]} )
    return v_out[0]
