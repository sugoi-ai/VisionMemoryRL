...
from constants import CHECKPOINT_MINESTONE
...
if __name__ == '__main__':
  ...
  for scene in scene_scopes:
    for task in list_of_tasks[scene]:
      branches.append((scene, task[0], task[1]))
  ...
  for i in range(PARALLEL_SIZE):
    scene, task ,checkpoint = branches[i%NUM_TASKS]
  ...
  for i in range(PARALLEL_SIZE):
    scene, task ,checkpoint = branches[i%NUM_TASKS]
    key = scene + "-" + task
    if task not in list_writer:
      list_writer.append(task)
      position = len(list_writer)-1
      tmp = tf.summary.FileWriter(str(LOG_FILE)+'/'+task, sess.graph)
      list_summary_writer.append(tmp)
    ...
    scalar_summaries = [
      tf.summary.scalar(scene+"/Episode Reward", episode_reward_input),
      tf.summary.scalar(scene+"/Episode Length", episode_length_input),
      tf.summary.scalar(scene+"/Episode Max Q", episode_max_q_input)
    ]

    summary_op[scene] = tf.summary.merge(scalar_summaries)
    summary_placeholders[scene] = {
  ...
  def train_function(parallel_index):
    ...
    scene, task ,checkpoint = branches[parallel_index % NUM_TASKS]
    key = scene + "-" + task

    for x in range(NUM_TASKS):
      if list_writer[x] == task:
        summary_writer = list_summary_writer[x]

    while global_t < MAX_TIME_STEP and not stop_requested:
      diff_global_t, isTerminal = training_thread.process(sess, global_t, summary_writer,
                                              summary_op[scene], summary_placeholders[scene])
      global_t += diff_global_t
      
      #if isTerminal:
      #  break

      if parallel_index == 0 and global_t - last_global_t > CHECKPOINT_MINESTONE:
