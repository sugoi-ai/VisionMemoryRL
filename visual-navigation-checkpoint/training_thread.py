...
class A3CTrainingThread(object):
  ...
  def process(self, sess, global_t, summary_writer, summary_op, summary_placeholders):
    ...
    for i in range(LOCAL_T_MAX):
      pi_, value_ = self.local_network.run_policy_and_value(sess, self.env.s_t, self.env.s_position, self.env.checkpoint, self.scopes)
    ...
    R = 0.0
    if not terminal_end:
      R = self.local_network.run_value(sess, self.env.s_t, self.env.s_position, self.env.checkpoint, self.scopes)
    ...
    for(ai, ri, si, Vi, ci, pi) in zip(actions, rewards, states, values, checkpoints, positions):
      R = ri + GAMMA * R
      td = R - Vi
